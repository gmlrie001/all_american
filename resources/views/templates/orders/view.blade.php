            ...
            <div class="col-12">
                <div class="row">
                    ...
                    <div class="col-12 col-lg-4 order-info-block">
                        <div>
                            <h2>Your Order</h2>
                            <p class="m-0"><span>Order Placed:</span>{{$order->order_placed}}</p>
                            <p class="m-0"><span>Sub Total:</span>R {{$order->subtotal}}</p>
                        @if($order->shipping_cost != null && $order->shipping_cost > 0)
                            <p class="m-0"><span>Shipping Cost:</span>R {{$order->shipping_cost}}</p>
                        @endif
                        @if($order->store_credit_value != null && $order->store_credit_value > 0)
                            <p class="m-0"><span>Store Credit Used:</span>-R {{$order->store_credit_value}}</p>
                            {{--<p class="m-0"><span>Store Credit Balance:</span>{{$order->store_credit_value}}</p>--}}
                        @endif
                        @if($order->coupon_value != null && $order->coupon_value > 0)
                            <p class="m-0"><span>Coupon:</span>-R {{$order->coupon_value}}</p>
                            <p class="m-0"><span>Coupon Code:</span>{{$order->coupon}}</p>
                        @endif
                            <p class="mx-0 mt-2 mb-0 py-1 font-weight-bold" style="border-top:1px solid #333;font-size:0.8rem;"><span class="w-50">Order Total:</span>R {{$order->total}}</p>
                            @php $tax = $order->total * ( 1.00 - ( 1.00 / 1.15 ) ); @endphp
                            <p class="m-0 pb-1 mb-2" style="border-bottom:1px solid #333;"><span>Tax:</span>R {{number_format( $tax, 2, ".", "" )}}</p>
                            <p class="m-0"><span>Total Items:</span>{{$order->products->count()}}</p>
                        @if($order->shipping_cost != null && $order->shipping_cost > 0)
                            <p class="m-0"><span>Shipping Method:</span>{{$order->shipping_title}}</p>
                            <p class="m-0"><span>Estimated Arrival:</span>{{$order->shipping_time_of_arrival}}</p>
                        @endif
                            <p class="m-0"><span>Payment Method:</span>{{$order->payment_method}}</p>
                        </div>
                    </div>
                </div>
            </div>
