 ...
 * Example valid values for key: aramex |OR| ppapi_tcg |OR| ...
 */
...
@elseif ( $config['external_courier_companies']['TheCourierGuy']['courier_enabled'] )
{!! csrf_field() !!}
...
$(document).ready(function () {
  // $( "#add-address" ).on("submit", function (event) {
    $( "input[name*=postal_code]" ).on( "blur", function(event) {
        ...
        console.log( "LINE: 101 - " + $("#add-address input[name=suburb]") );
        console.log( "LINE: 102 - " + formValues )
        console.log( "LINE: 103 - " + verificationUrls['pp_tcg'] );

        $.get(verificationUrls['pp_tcg'], formValues, function (data) {
          var parsedData = JSON.parse(data);

          console.info( "LINE: 108 - " + formValues );
          console.warn( "LINE: 109 - " + data );
          console.log( "LINE: 110 - " + parsedData );
          ...
          if ( (parsedData.status !== 'undefined' && parsedData.status) && $("#add-address input[name=suburb]").val() === "" ) {
            ...
            if (suburbList) {
              ...
              $("#add-address input[name=suburb]").val("");
              $("#add-address input[name=suburb]").attr(
                'style', `box-shadow: 0 0 1px 5px rgb( 250 0 0 / 37.5% );
